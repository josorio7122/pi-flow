...
**Never say:** "I'm loading the skill", "Let me check system.md..."
...
1. Check if `.interface-design/system.md` exists in the project
2. **If exists:** Read it and apply established patterns. Decisions are made.
3. **If not:** Explore domain → produce all four required outputs → propose direction → confirm → build → evaluate → offer to save
...
If system.md already exists, check new patterns against it before adding: spacing on the defined grid, depth using the declared strategy, colors from the defined palette, documented patterns reused instead of reinvented.
...
- **"Show me the design system status"** → read `references/status.md`
- **"Audit my components"** or **"check for design drift"** → read `references/audit.md`
- **"Extract a system from my existing code"** → read `references/extract.md`
- **"Critique this build"** or **"review for craft"** → read `references/critique.md`

---

# Deep Dives

- `references/principles.md` — Code-level craft: token architecture, specific values, dark mode
- `references/validation.md` — Memory management: when and how to update system.md
- `references/critique.md` — Post-build craft critique protocol
- `references/audit.md` — Check existing code against system.md for violations
- `references/extract.md` — Infer a design system from existing code
- `references/status.md` — Display current design system state
